...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .case-score-head {{ display:grid; grid-template-columns:170px minmax(0,1fr); gap:10px; margin-bottom:14px; }}
    .card {{ padding:15px; }}
    ...
    .effort-list {{ display:grid; gap:8px; }}
    .effort-turn {{ border:1px solid var(--line); border-radius:var(--radius); padding:10px; background:#fff; display:grid; grid-template-columns:92px minmax(0,1fr); gap:10px; }}
    ...
    .scores {{ display:grid; grid-template-columns:repeat(6,1fr); gap:9px; }}
    .dim {{ border:1px solid var(--line); border-radius:var(--radius); padding:10px; background:#fff; }}
    @media(max-width:1100px) {{ header,.layout,.case-score-head,.sections,.rounds,.memory,.scores {{ display:grid; grid-template-columns:1fr; }} .stats {{ min-width:0; grid-template-columns:repeat(2,1fr); }} }}
  </style>
</head>
<body>
<div class="page">
  <header>
    <div><h1>Assist Everything BetterAndBetter Eval Workbench</h1><div class="sub">自动 eval 结果：三轮 Case、记忆状态跃迁、用户费力度下降与六维评分。</div></div>
    <div class="stats">
  ...
  <div class="layout">
    ...
    <main>
      <section class="case-score-head"><div class="stat"><span class="muted">当前 Case 总分</span><b id="case-score"></b><span class="muted" id="score-case-id"></span></div><div class="card"><h2>Case 六维评分</h2><div id="scores" class="scores"></div></div></section>
      <section class="card"><div class="row"><div><h2 id="title"></h2><div id="goal" class="muted"></div></div></div><div id="rounds" class="rounds"></div></section>
      <section class="sections"><div class="card"><h2>Memory 状态跃迁</h2><div id="memory" class="memory"></div></div><div class="card"><h2>用户费力度轨迹</h2><div id="effort-list" class="effort-list"></div></div></section>
    </main>
  </div>
</div>
<script>
...
function render() {{
  ...
  document.getElementById('score-case-id').textContent = c.id;
  ...
  document.getElementById('memory').innerHTML = (c.memory_journey || memoryJourney(c)).map(m => `<div class="mem"><b>${{m.state}} ${{m.title}}</b><div class="muted">${{m.reason}}</div><div class="bar"><span style="width:${{m.effort_score}}%"></span></div><div class="muted">${{m.effort_level}}费力度：${{m.effort_score}}</div></div>`).join('');
  const effort = c.user_effort || {{turns:[]}};
  document.getElementById('effort-list').innerHTML = (effort.turns || []).map(e => `<div class="effort-turn"><div><b>${{e.turn_id}}</b><br><span class="${{e.delta > 0 ? 'chip red' : 'chip green'}}">${{e.delta > 0 ? '+' : ''}}${{e.delta}}</span><div class="muted">${{e.before}}→${{e.after}}</div></div><div><b>${{e.stage}}</b><div class="muted">用户：${{e.user}}</div><div class="body">${{(e.reasons || []).join('；')}}</div><div class="chips">${{(e.six_dim_gain || []).map(x => `<span class="chip">${{x}}</span>`).join('')}}</div></div></div>`).join('');
...
function memoryJourney(c) {{
  const turns = (c.user_effort && c.user_effort.turns) || [];
  if (!turns.length) return [{{state:'M0', title:'无费力度轨迹', reason:'未生成用户费力度分析。', effort_score:85, effort_level:'高'}}];
  return turns.filter((_, i) => i === 0 || i === Math.floor(turns.length / 2) || i === turns.length - 1).map(e => {{
    const level = e.after >= 70 ? '高' : (e.after >= 40 ? '中' : '低');
    return {{state:e.turn_id, title:e.stage, reason:(e.reasons || []).join('；'), effort_score:e.after, effort_level:level}};
  }});
}}
render();
</script>
